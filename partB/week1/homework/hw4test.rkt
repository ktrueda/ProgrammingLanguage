...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 6 5 1) null "Sequence test")
   ...
   (check-equal? (string-append-map 
                  null
                  ".jpg") null "string-append-map test")
   ...
   (check-equal? (vector-assoc 5 (vector (cons 4 3) 5 (cons -5 3))) #f "vector-assoc test")
   (check-equal? (vector-assoc 5 (vector)) #f "vector-assoc test")
   ...
   ; while-less test
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) (print "A") a)) #t "while-less test")
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) (print "B") a)) #t "while-less test")
   ;
   ))
